function main() {
  ...
  if (
    url.includes(".gelbooru.com/images/") ||
  ...
  const artists = [];
  const artistLi = document.querySelectorAll(".tag-type-artist");
  ...
  if (!(artistLi.length == 0)) {
    ...
    if (url.includes("gelbooru.com")) {
      ...
      artists.forEach((artist) => {
        if (document.getElementById("tags").value.includes(artist)) def = "l";
      });
    }
  }
...
k - sort tabs by number of artist renditions",
    def,
  );
