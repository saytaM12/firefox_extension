...
async function getAllTabs() {
  ...
  return tabs.filter(
    (tab) =>
      tab.url.includes("gelbooru") ||
      tab.url.includes("rule34") ||
      tab.url.includes("lolibooru") ||
      tab.url.includes("yande.re"),
  );
}
...
function downloadAll() {
  getAllTabs().then((tabs) => {
    ...
    tabs.forEach((tab) => {
      url = tab.url;
      if (tab.url.includes(".gelbooru.com/images/")) {
        download = true;
  ...
  getCurrentTab().then((tab) => {
    browser.scripting.executeScript({
      ...
      func: (string) => {
        ...
        const textBox = document.getElementById("tags-search");
        modifiers.forEach((modifier) => {
          textBox.value = textBox.value + " " + modifier;
        });
...
function tabError(tab) {
  if (tab.url.includes("gelbooru") && tab.url.includes(tab.title)) {
    return true;
...
function sortBySize() {
  ...
  setTimeout(() => {
    const sortedTabs = renditions.sort((a, b) => {
      return b.count - a.count;
    });
    const tabIds = sortedTabs.map((tabWithCount) =>
      parseInt(tabWithCount.tabId),
    );
    ...
    for (const tabId of tabIds) {
      ...
      console.log(
        ...
        tabId,
        "to index",
        tabCount - tabIds.indexOf(tabId) - 1,
      );
...
browser.runtime.onMessage.addListener((m, sender) => {
  if (m.type == "action" && m.text != null) {
    ...
    for (let i = 0; i < m.text.length; i += 1) {
      if (processed.includes(m.text[i])) continue;
      processed += m.text[i];
      switch (m.text[i]) {
        case "s":
